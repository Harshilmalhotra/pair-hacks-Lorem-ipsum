...
nlp_multilingual = spacy.load("xx_ent_wiki_sm")  # Multilingual model
...
def get_spacy_model(language_code):
    # Define the language codes and their models
    if language_code == 'en':
        ...
        if block["type"] == 0:  # This indicates the block contains text
            for line in block["lines"]:
                for span in line["spans"]:
                    ...
                    for ent in doc.ents:
                        # Only mask names, emails, and addresses
                        if ent.label_ in ["PERSON", "EMAIL", "ADDRESS"]:
                            print(f"Masking {ent.label_}: {ent.text}")
                            # Mask the entity by drawing a black rectangle
